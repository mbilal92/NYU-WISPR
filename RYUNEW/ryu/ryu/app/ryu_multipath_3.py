    ...
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst, src_mac ,dst_mac):
        ...
        for path in paths_with_ports:
            ...
            for node2 in MirriorPath:
                    ...
                    if node2 == src:
                        match2 = parser.OFPMatch(in_port=2, eth_dst=dst_mac, eth_src=src_mac, eth_type=0x8100, vlan_vid=mirrIorvlan)
                        self.add_flow(dp, 3, match2, actions)
                    elif node2 == dst:
                        match3 = parser.OFPMatch(in_port=in_port, eth_dst=dst_mac, eth_src=src_mac, eth_type=0x8100, vlan_vid=mirrIorvlan)
                        self.add_flow(dp, 3, match3, actions2)
                    else:
                        match = parser.OFPMatch(in_port=in_port, eth_dst=dst_mac, eth_src=src_mac, eth_type=0x8100, vlan_vid=mirrIorvlan)
                        self.add_flow(dp, 3, match, actions)
