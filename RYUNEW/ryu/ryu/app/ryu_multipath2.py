    ...
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst):
        ...
        for path in paths_with_ports:
            for node in path:
                    ...
                    # if firstPath:
                    #     self.add_flow(dp, 1, match_arp, actions)
                    #     print node, in_port, out_port , "arp"

                    if node == src:
                        match = parser.OFPMatch(in_port=in_port, ipv4_src=ip_src, ipv4_dst=ip_dst, eth_type=0x0800)
                        self.add_flow(dp, 2, match, actions2)

                        match2 = parser.OFPMatch(in_port=2, ipv4_src=ip_src, ipv4_dst=ip_dst, eth_type=0x8100, vlan_vid=vlan)
                        self.add_flow(dp, 2, match2, actions)

                        # print node, in_port, out_port , "SRC"

                    elif node == dst:
                        match3 = parser.OFPMatch(in_port=in_port, ipv4_src=ip_src, ipv4_dst=ip_dst, eth_type=0x8100, vlan_vid=vlan)
                        self.add_flow(dp, 2, match3, actions2)

                        match4 = parser.OFPMatch(in_port=2, ipv4_src=ip_src, ipv4_dst=ip_dst, eth_type=0x0800)
                        self.add_flow(dp, 2, match4, actions)

                        # print node, in_port, out_port , "Dst"

                    else:
                        match = parser.OFPMatch(in_port=in_port, ipv4_src=ip_src, ipv4_dst=ip_dst, eth_type=0x8100, vlan_vid=vlan)
