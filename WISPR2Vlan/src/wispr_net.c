...
struct wisprsock *init_wispr_sockets(char *interface, char *interface2, char *gateWayIporMAC) {
  ...
  strncpy(ifr.ifr_name, wisprs->ifName, sizeof(ifr.ifr_name));

  if (ioctl(rawsd, SIOCGIFMTU, &ifr) == -1) {
    daemon_log(LOG_INFO,"Error opening:");
  } else {
    daemon_log(LOG_INFO, "MTU: %d\n",ifr.ifr_mtu);
    mtu = ifr.ifr_mtu;
  }
  
  // ifopts.ifr_mtu = 65535; // Change value if it needed

  // if(!ioctl(rawsd, SIOCSIFMTU, &ifopts)) {
  //   daemon_log(LOG_INFO, "MTU changed\n");
  // } else {
  //   daemon_log(LOG_INFO, "MTU Error\n");
  // }

  // if(!ioctl(sockfd, SIOCSIFMTU, &ifopts)) {
  //   daemon_log(LOG_INFO, "MTU changed 2\n");
  // } else {
  //   daemon_log(LOG_INFO, "MTU Error 2\n");
  // }

  // ioctl(rawsd, SIOCSIFMTU, &ifopts);
  // ioctl(sendfd , SIOCSIFMTU, &ifopts);
  ...
  * prematurely */
  ...
  int socket_mtu = IP_PMTUDISC_DONT;
  int mtu = setsockopt(rawsd, IPPROTO_IP, IP_MTU_DISCOVER, &socket_mtu, sizeof(socket_mtu));
  daemon_log(LOG_INFO, "MTU %d", mtu);
  // setsockopt(sendfd, IPPROTO_IP, IP_MTU_DISCOVER, &socket_mtu, sizeof(socket_mtu));
...
void handle_vlanPkt(uint8_t *buf, ssize_t numbytes, struct wisprsock *wisprs) {
          ...
          // case 0: // WISPR 0 - STRIPING
            process_wispr0_vlan(buf, numbytes, wisprs, mtu);
          //   break;
...
void handle_ipv4pkt(uint8_t *buf, ssize_t numbytes, struct wisprsock *wisprs) {
  ...
  // daemon_log(LOG_INFO, "%.2X-%.2X-%.2X-%.2X-%.2X-%.2X \n", eth->h_source[0] , eth->h_source[1] , eth->h_source[2] , eth->h_source[3] , eth->h_source[4] , eth->h_source[5] );
    ...
    // if (strcmp(source, "10.0.0.1") == 0 || strcmp(source, "10.0.0.2") == 0 || strcmp(source, "192.168.2.128") == 0)
    // // if (strstr(&source, "10.0.0"))
    // {
      // daemon_log(LOG_INFO, "%.2X-%.2X-%.2X-%.2X-%.2X-%.2X \n", eth->h_dest[0] , eth->h_dest[1] , eth->h_dest[2] , eth->h_dest[3] , eth->h_dest[4] , eth->h_dest[5]);
      // daemon_log(LOG_INFO, "%.2X-%.2X-%.2X-%.2X-%.2X-%.2X \n", eth->h_source[0] , eth->h_source[1] , eth->h_source[2] , eth->h_source[3] , eth->h_source[4] , eth->h_source[5] );
      process_ingress(buf, numbytes, wisprs, 0, mtu);
    // } else {
...
int send_packet_raw(uint8_t *buf, ssize_t numbytes, uint16_t sendsd) {
 ...
 //   daemon_log(LOG_INFO, tempDest);
  ...
  // if ( (i = write(sendsd, buf, numbytes) < 0) ) {
    ...
    daemon_log(LOG_INFO, "VLAN sent bytes RAW %d", i);
    return i;
  }
}
