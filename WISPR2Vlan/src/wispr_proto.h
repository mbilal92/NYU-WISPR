...
#define PACKET_SIZE 66000
#define WISPR5ENTRIES 4096
// #define WISPR1ENTRIES 65536
#define WISPR1ENTRIES 16

#define NUMBEROFPATHS 5
#define NUMBEROFGROUPS 2

#define ETHER_TYPE 0x0800
#define GRE_HEADER_SECTION 4
#define KEY_SIZE 4
#define TIMEOUT                                                                \
  600 // difference in microsends to determine whether table values are
          // fresh
...
typedef struct {
  wispr_key_t key;
  struct timeval tv[NUMBEROFGROUPS][NUMBEROFPATHS+1]; //  NUMBEROFPATHS+1 because Vlan tagging start from 1
  UT_hash_handle hh;
} wisprmirror_ht;
...
void process_ingress(uint8_t *buf,  ssize_t numbytes, struct wisprsock *wisprs, int mode, int mtu);
...
int send_packet_withkey(uint8_t *, ssize_t, uint16_t, wispringress_ht_vlan *);
...
wispringress_ht_vlan * get_or_add_ingresssession(wispr_key_t);
