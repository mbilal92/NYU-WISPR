...
#define KEY_SIZE 4
#define TIMEOUT                                                                \
  5000000 // difference in microsends to determine whether table values are
          // fresh
#define PARITYTIMEOUT 50000000
#define NUMBEROFPATHS 500

// wisprmirror_ht *wisprmirt;
// wisprparity_ht *wisprpart;
wispringress_ht *wispringress;
host_IP *hostIp;
...
void process_ingress0(uint8_t *buf, ssize_t numbytes, struct wisprsock *wisprs) {
  ...
  wispringress_ht *tmp, *mirror_tmp;
  ...
  if (check_dst_ip_host(key.dst_ip) == -1) {
    ...
    if (tmp != NULL) {
      tmp->wispr_key.bitmask += 1;
      if (tmp->wispr_key.bitmask == NUMBEROFPATHS)
      {
        tmp->wispr_key.index += 1;
        tmp->wispr_key.bitmask = 0; 
      }
    } else {
      tmp->wispr_key.index = 0;
      tmp->wispr_key.bitmask = 0;
      tmp->wispr_key.mode = 0;
    }
...
wispringress_ht * get_or_add_ingresssession(wispr_key_t key) {
  daemon_log(LOG_INFO, __func__);
  wispringress_ht *new;
  ...
  if (new == NULL) {
    ...
    new = (wispringress_ht *)malloc(sizeof(wispringress_ht));
    memset(new, 0, sizeof(wispringress_ht));
    new->key = key;
...
int send_packet_withkey(uint8_t *buf, ssize_t numbytes, uint16_t sendsd,
  ...
  int i;
  char str[numbytes + sizeof(struct wispr_packet)];
  ...
  uint8_t *rem_buf;
  int rem_len = numbytes - sizeof(struct iphdr) - sizeof(struct ether_header);
  rem_buf = (uint8_t * ) malloc(rem_len);
  memcpy(rem_buf, buf + sizeof(struct iphdr) + sizeof(struct ether_header), rem_len);

  // memset(buf + sizeof(struct iphdr) + sizeof(struct ether_header), 0, rem_len);
  struct wispr_packet *gre = (struct wispr_packet *)(buf + sizeof(struct iphdr) + sizeof(struct ether_header));
  uint16_t temp_gre_flags = gre->header.gre_flags;
  // gre->header.gre_flags = htons(ntohl(temp_gre_flags) | GRE_FLAGS_KP);
  gre->header.gre_flags = htons(GRE_FLAGS_KP);
  gre->key = htons(key->wispr_key);
  gre->header.gre_proto = htons(0x0000);
  // gre->header.gre_proto = htons(ETH_P_IP);
  // gre->header.gre_proto = ntohl(ETHER_TYPE);
  unsigned int *buffer = (unsigned int*)&gre->key;
  daemon_log(LOG_INFO, "0x%x -- %d\n", buffer[0], buffer[0]) ;
  daemon_log(LOG_INFO,"%s\n", int2bin(buffer[0], NULL));

  daemon_log(LOG_INFO,"GRE THINGS: %d %d %d %d %d\n", gre->header.gre_flags, gre->header.gre_proto, gre->key.mode, gre->key.index, gre->key.bitmask);
  memcpy(buf + sizeof(struct ether_header) + sizeof(struct iphdr) + sizeof(struct wispr_packet),
      rem_buf, rem_len);
...
// daemon_log(LOG_INFO, "\n");
  ...
  sentbytes = sendto(sendsd, buf , numbytes + sizeof(struct wispr_packet) , 0, NULL, NULL);
